...
float edgeFade(vec2 frag, vec2 res, vec2 offset){
    ...
    s = mix(s, tail, 0.2);
    float dn = (layeredNoise(frag * 0.15) - 0.5) * 0.002 * s;
...
void main(){
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        float stepLen = min(rad - 0.3, n * jitterAmp) + 0.25;
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    const update = now => {
      ...
      const visible = isVisibleRef.current && !document.hidden && !isLightMode;
