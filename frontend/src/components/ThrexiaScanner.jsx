...
void main() {
  ...
  // SECURE THEME COLORS - NO EXTERNAL LEAKS
  vec3 th_purple = vec3(0.54, 0.36, 0.96); // #8B5CF6
  vec3 th_blue   = vec3(0.23, 0.51, 0.96); // #3B82F6

  vec3 baseColor = th_purple * (ringGlow + spokeGlow);
  vec3 sweepColor = th_blue * sweepBeam;
  
  vec3 col = (baseColor + sweepColor) * fade * uBrightness;
...
export default function ThrexiaScanner({
  speed = 1.0,
  scale = 0.5,
  ringCount = 10.0,
  spokeCount = 10.0,
  ...
  }, [speed, scale, ringCount, spokeCount, ringThickness, spokeThickness, sweepSpeed, sweepWidth, sweepLobes, falloff, brightness, enableMouseInteraction, mouseInfluence]);

  return <div ref={containerRef} className="threxia-scanner-container" />;
}
